...
(($) ->
  $.fn.tablefix = (options) ->
    @each (index) ->
      ...
      bodyDiv.width(bodyWidth + ((if withWidth then 5 else 5)) + ((if withHeight then 15 else 0))).height(bodyHeight + ((if withWidth then 0 else 0)) + ((if withHeight then 0 else 20))).css
      # bodyDiv.width(bodyWidth + 50).height(bodyHeight + 50).css
        left: offsetX + "px"
        top: offsetY + "px"
