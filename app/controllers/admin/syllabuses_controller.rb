class Admin::SyllabusesController < ApplicationController
  ...
  def show
    @syllabus = Admin::Syllabus.find(params[:id])
  ...
  def create
    @syllabus = Admin::Syllabus.new(params[:syllabus])

    respond_to do |format|
      if @syllabus.save
        format.html { redirect_to admin_syllabus_url(@syllabus), notice: 'Syllabus was successfully created.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_syllabuses_url }
      format.json { head :ok }
    end
  end
end
