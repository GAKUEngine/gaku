...
describe 'Exam Notes' do
  ...
  context 'new' do
    before do 
      ...
      wait_until { find("#submit-exam-note-button").visible? }
    end

    it "should add and show exam note", :js => true do
      @exam.notes.size.should eql(0)
      tr_count = page.all('table#exam-notes-index tr').size
 
      fill_in "note_title", :with => "The note title"
      fill_in "note_content", :with => "The note content"
      click_button "submit-exam-note-button"

      wait_until { !page.find('#new-note form').visible? } 
      page.should have_selector('a', href: "/exams/1/notes/1/edit")
      page.should have_content("The note title")
      page.should have_content("The note content")
      page.all('table#exam-notes-index tr').size == tr_count + 1
      within('.exam-notes-count') { page.should have_content('Notes list(1)') }
      @exam.reload
      @exam.notes.size.should eql(1)
    end
  ...
  context "edit and delete" do 
    ...
    it "should edit a exam note", :js => true do 
    ...
    it 'should cancel editting', :js => true do
      ...
      wait_until { find('#edit-note-modal').visible? }
      click_link 'cancel-exam-note-link'
      wait_until { !page.find('#edit-note-modal').visible? }
    end

    it "should delete a exam note", :js => true do
      @exam.notes.size.should eql(1)
      ...
      wait_until { page.all('table#exam-notes-index tr').size == tr_count - 1 } 
      within('.exam-notes-count') { page.should_not have_content('Notes list(1)') }
